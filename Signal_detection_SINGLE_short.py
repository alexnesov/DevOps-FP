...
def SignalDetection(df, tick, *args):
    ...
    df['doubleSignal'] = np.where(
        (df["Aroon Up"] < df["Aroon Down"]) & (df['positions']==1) & (df["Aroon Down"]>55) &(df["Aroon Up"]<75),
        1,0)
...
def csvAppend(df):
    ...
    if init == True:
        ...
        df.to_csv('single.csv', mode='a', index=False, header=False)
...
def main():
    print(f'Main: for {tick}')
    SE = 'NYSE'
    qu = f"SELECT * FROM {SE}_15 WHERE Symbol='{tick}' AND DATE > '2020-01-01'"
